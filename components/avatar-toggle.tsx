 ...
 * Статичные day.PNG / night.PNG — это РОВНО первый и последний кадр гифки,
...
export function AvatarToggle({
  ...
  // (она уже в памяти) и НИКОГДА не качается по сети повторно.
  React.useEffect(() => {
    setMounted(true);
  ...
  React.useEffect(() => {
    ...
    // Подложка во время анимации = СТАРТОВЫЙ кадр (свет→тьма стартует «в очках»).
